...
const pinnedPackage = `apple-health-mcp-unofficial@${packageJson.version}`;
const exportPath = resolve('fixtures/apple_health_export/export.xml');
const home = mkdtempSync(join(tmpdir(), 'apple-health-mcp-hermes-'));

const client = new Client({ name: 'apple-health-mcp-hermes-agent-test', version: '0.0.0' });
const transport = new StdioClientTransport({ command: 'node', args: ['dist/index.js'] });

try {
  await client.connect(transport);
  const manifestResult = await client.callTool({
    name: 'apple_health_agent_manifest',
  ...
  assert.ok(setupPayload.hermes_skill_path.endsWith('.hermes/skills/apple-health-mcp/SKILL.md'));
  ...
  assert.equal(doctorPayload.client_checks.hermes.apple_health_server_configured, true);
  ...
  const mergeHome = mkdtempSync(join(tmpdir(), 'apple-health-mcp-hermes-merge-'));
  ...
  assert.match(mergedConfig, /apple_health:/);
