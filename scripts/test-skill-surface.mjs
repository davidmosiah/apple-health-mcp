...
assert.match(skill, /call apple_health_connection_status/);
...
const result = await run(["call", "apple_health_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "apple_health_connection_status", files: pkg.files }, null, 2));
