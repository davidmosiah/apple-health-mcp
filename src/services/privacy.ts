...
export function recordPrivacyView(records: AppleHealthRecord[], mode: PrivacyMode, timezone = "UTC") {
  if (mode === "raw") {
    return {
      records,
...
export function recordAggregate(records: AppleHealthRecord[], timezone = "UTC") {
  const numeric = records.filter((record) => record.numeric_value !== undefined) as Array<AppleHealthRecord & { numeric_value: number }>;
