...
import { getExportSnapshot, parseAppleDate } from "./apple-health-export.js";
...
function latestDate(values: Array<string | undefined>): Date | undefined {
  return values
    .map((value) => parseAppleDate(value))
...
function earlier(left: string | undefined, right: string | undefined): string | undefined {
  ...
  const leftDate = parseAppleDate(left);
  const rightDate = parseAppleDate(right);
