/**
 * Watch-folder auto-reimport for the local Apple Health export.
 *
 * Apple Health is a manual export: the user taps "Export All Health Data" on the
 * iPhone and drops the resulting `export.zip` somewhere on disk. Without a watch
 * folder this connector only ever reads a single fixed path, so a fresh export
 * is invisible until the user re-runs `setup`.
 *
 * The watch folder turns that into a recurring workflow: point the connector at
 * a directory (via `APPLE_HEALTH_WATCH_PATH`, local config, or
 * `setup --watch-path <dir>`). Whenever a newer Apple Health export shows up in
 * that folder, `reconcileWatchFolder` promotes it to be the active export
 * (`APPLE_HEALTH_EXPORT_PATH`), drops the in-memory snapshot cache, and
 ...
 * is also exposed as the `apple_health_reimport` MCP tool for an explicit
 * manual re-scan.
 *
 * This is the cross-platform path. The native HealthKit bridge (live, no manual
 * export) genuinely needs macOS and is out of scope here.
 */
import { existsSync, readdirSync } from "node:fs";
import { homedir } from "node:os";
import { extname, join, resolve } from "node:path";
import { clearSnapshotCache, inspectExportLocation, type ExportLocation } from "./apple-health-export.js";
...
export interface WatchCandidate {
  /** The path that would be set as APPLE_HEALTH_EXPORT_PATH. */
  path: string;
...
/**
 * Scan the watch folder for the freshest Apple Health export. Supports the same
 * shapes as `inspectExportLocation`: a top-level `export.xml`, `export.zip`, an
 * `apple_health_export/` directory, or any `*apple*health*.zip`. The newest by
 * file mtime wins.
 */
export async function findLatestExportInFolder(watchPath: string): Promise<WatchCandidate | undefined> {
  ...
  for (const candidatePath of rawCandidatePaths) {
    ...
    verified.push({
      ...
      export_xml_path: location.export_xml_path,
      kind: location.kind,
...
function collectCandidatePaths(path: string, depth: number, out: string[]): void {
  ...
  for (const entry of entries) {
    ...
    if (lower === "export.xml" || lower === "export.zip") out.push(fullPath);
    else if (lower.includes("apple") && lower.includes("health") && extname(lower) === ".zip") out.push(fullPath);
  }
}

/**
 * Check the watch folder for a newer export than the currently-active one and,
 * if found, promote it: persist the new `APPLE_HEALTH_EXPORT_PATH`, record the
...
export async function reconcileWatchFolder(
  ...
  if (!config.watchPath) {
    return {
      ...
      message: "No watch folder configured. Set APPLE_HEALTH_WATCH_PATH or run `setup --watch-path <dir>`."
    };
  }
  ...
  if (!watchPath || !existsSync(watchPath)) {
    return {
      ...
      active_export_path: previousExportPath,
      previous_export_path: previousExportPath,
      message: `Watch folder does not exist: ${config.watchPath}. Create it or update APPLE_HEALTH_WATCH_PATH.`
    };
  }
  ...
  if (!candidate) {
    return {
      ...
      active_export_path: previousExportPath,
      previous_export_path: previousExportPath,
      message: `No Apple Health export (export.xml, export.zip, or apple_health_export/) found in ${watchPath}.`
    };
  }
  ...
  mergeLocalConfig(
    {
      APPLE_HEALTH_EXPORT_PATH: candidate.path,
      APPLE_HEALTH_LAST_WATCH_IMPORT_PATH: candidate.path,
      APPLE_HEALTH_LAST_WATCH_IMPORT_AT: reimportedAt
    },
    homeDir
  );
  ...
  return {
    ...
    message:
      reason === "promoted_new_export"
        ? `Promoted new Apple Health export from watch folder: ${candidate.path}. Summaries now reflect this export.`
        : `Re-imported updated Apple Health export: ${candidate.path}. Summaries refreshed.`
  };
}
